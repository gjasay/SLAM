...
namespace slam::ui {
 Style StyleManager::Resolve(const Element* e) const {
   ...
   if (tagIt != rules.end()) merge(out, tagIt->second);

   for (const auto& cls : e->classes) {
     auto clsIt = rules.find("." + cls);
     if (clsIt != rules.end()) merge(out, clsIt->second);
   }

   if (!e->id.empty()) {
     auto idIt = rules.find("#" + e->id);
     if (idIt != rules.end()) merge(out, idIt->second);
   }
  ...
  void StyleManager::merge(Style& base, const Style& add) {
    ...
    if (add.borderWidth   != 0)        base.borderWidth   = add.borderWidth;
    if (add.padding       != 0)        base.padding       = add.padding;
    if (add.backgroundColor.a != 0)    base.backgroundColor = add.backgroundColor;
    if (add.borderColor.a     != 0)    base.borderColor     = add.borderColor;
  }
